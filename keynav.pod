...
To daemonize on startup:
 keynav daemonize

=head1 CONFIGURATION

keynav is configured from a config file in your home directory ".keynavrc"
...
=item B<grid-nav> I<[on OR off]>

Grid navigation is off by default. When turned on, every grid cell will have a
two-letter label. To select a single cell, you simply type the two letters.
...
simply press 'l' while the keynav window is active.
...
This manual page was written originally by INSERT DEBIAN MAINTAINER
E<lt>...@...<gt> for the Debian project (but may be used by others). It is
